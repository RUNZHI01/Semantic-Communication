...
**（3）variance family：stage folding 与逐通道 local handoff。** MetaSchedule 生成的方差计算按"逐元素先减均值、再平方、再归约"的流程展开，需要分配两个全帧 `(1,12,256,256)` 的中间缓冲来存放居中值（`T_subtract`）和平方值（`T_multiply`），加上一次全帧读取计算平方和。对于单个通道，3 MB 的输入张量会产生约 15 MB 的中间读写，远超 Cortex-A72 的 32 KB L1d 缓存容量。
...
**baseline 的问题。** MetaSchedule 生成的实现将 mean4 分解为 7 个独立 pass：reduction → divide（求均值）→ subtract（逐元素减均值）→ divide（除标准差）→ multiply（乘 weight）→ add（加 bias）→ relu。其中 4 个 pass 需要分配全帧 `(1,12,256,256)` 的中间缓冲：
...
每个通道的 3 MB 输入需要约 31 MB 的总读写流量（4 个全帧中间 buffer 各读写一次加上输入/输出），远超 Cortex-A72 的 32 KB L1d 缓存，导致大量 cache miss。

**v4：逐通道参数 hoist 与 epilogue 融合。** 观察到 mean、std、weight、bias 四个参数在通道内均为常量，v4 将它们 hoist 到 local buffer 中逐通道加载一次，并将 5 个全帧 elementwise pass 融合为单趟内循环：
...
**v5：affine pair 预计算。** 进一步观察到，内循环中的 `(x - mean) / std * weight + bias` 可以通过代数化简为 `x * scale + shift`，其中 `scale = weight / std`，`shift = bias - mean * scale`。这两个值在通道内为常量，可在进入 256×256 热循环前预计算一次：
...
**v5/v6 的 codegen 检查与方向校正。** 为了确定下一步优化方向，对 v5 产物的 AArch64 反汇编进行了检查。结果表明，epilogue（affine + ReLU）已经被 LLVM 成功降低为 NEON 向量指令：
...
**v7：reduction 侧 4-lane NEON 向量归约。** 基于上述 codegen 分析，v7 在保留 v5 affine epilogue 不变的前提下，仅改写 reduction 结构。核心做法是将 256 宽的内层 reduction 拆分为 `64 × 4`，内层 4 元素标记为 `T.vectorized`，使 TVM 的 LLVM 后端生成 4-lane 向量累加：
...
至此，mean4 的热路径从 baseline 的"7 个独立 pass + 标量全路径"，经过 v4（参数 hoist）、v5（affine 预计算）、v7（向量归约）三步迭代，最终达到"reduction 侧和 epilogue 侧均为 NEON 向量化"的状态。表 4.2 总结了这一演进过程中的 codegen 状态变化：

| 版本 | reduction 侧 codegen | epilogue 侧 codegen | 关键改写 |
|-----|----------------------|---------------------|---------|
| baseline | 标量 | 标量（7 个独立 pass） | — |
| v4 | 标量 | 标量（单趟融合） | 参数 hoist + pass 融合 |
| v5 | 标量（`fadd s1, s1, s2` 链） | NEON（`dup` + `fmla` + `fmaxnm`） | affine pair 预计算 |
| v7 | NEON（`fadd v0.4s` + `faddp`） | NEON（同 v5） | 4-lane partial-sum 向量归约 |

> **表 4.2** mean4 各版本的 AArch64 codegen 状态演进
...
│                                                                  │
│  ┌──────────────┐          ┌──────────────┐                     │
│  │ Preload Proc │          │ Infer Proc   │                     │
│  │ (小核 2,3)   │          │ (大核 0,1)   │                     │
│  │              │          │              │                     │
│  │ read latent ──┼──queue──► TVM VM Exec  │                     │
│  │   from disk  │          │   (主要瓶颈)  │                     │
│  └──────────────┘          └──────┬───────┘                     │
│         ▲                          │                             │
│         │                    ┌─────┴─────┐                       │
...
5. **ACL 路线仍处于探索态。** 当前 ACL 单热点替换线主要用于验证转置卷积算子的调用可行性，尚未完成多算子覆盖的整图级集成，因此相关结果应视为探索性对比，而非成熟替代方案。

"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose_add6.
...
- it does not introduce a raw pre-compile handwritten lane for transpose_add6
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in transpose_add6 scheduled-form v1 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the transpose_add6 scheduled-form v1 working copy can be consumed by the existing local post-db path.",
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Use run_transpose_add6_post_db_local_build.py to consume this checked-in "
            "transpose_add6 v1 working copy through the local post-db path."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in transpose_add6 scheduled-form v1 working copy is consumed only through the local post-db scheduled swap seam here.",

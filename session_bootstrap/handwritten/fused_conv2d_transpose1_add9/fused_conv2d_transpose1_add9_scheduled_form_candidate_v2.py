"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose1_add9 v2.

This module points the local schedule-preserving seam at a new v2 working copy
that layers a P1-style dilate+pad fusion attempt on top of the accepted v1/P2/P4
state without mutating the accepted baseline files.
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in scheduled-form v2 working copy, so resulting evidence is "
        "local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the scheduled-form v2 working copy can be consumed by the existing local schedule-preserving seam.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in scheduled-form v2 working copy is consumed only through the local post-db scheduled swap seam here.",

"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose1_add9.
...
- it does not replace the older raw pre-compile hook-facing candidate module
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in scheduled-form v1 working copy, so resulting evidence is "
        "local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the scheduled-form v1 working copy can be consumed by the existing local schedule-preserving seam.",
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Use run_transpose1_post_db_local_build.py or "
            "run_transpose1_post_db_local_build_and_sync.py to consume this "
            "checked-in v1 working copy through the local post-db path."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in scheduled-form v1 working copy is consumed only through the local post-db scheduled swap seam here.",

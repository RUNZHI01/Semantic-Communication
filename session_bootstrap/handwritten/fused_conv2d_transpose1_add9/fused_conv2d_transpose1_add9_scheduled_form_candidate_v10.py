"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose1_add9 v10.

This module points the local schedule-preserving seam at a v10 working copy
that keeps the winning v7 stripe shape, dc_0 slice staging, and c_1-before-w_1
consumer order intact while routing the second h_1 stripe through a disjoint
consumer-facing current buffer instead of rewriting the shared data_pad rows in
place.
"""
...
WORKING_COPY_TIR_FILENAME = (
    f"{OPERATOR_NAME}_scheduled_form_candidate_v10_working_copy_tir.py"
)
WORKING_COPY_MANIFEST_FILENAME = "scheduled_form_candidate_v10_working_copy_manifest.json"
CANDIDATE_VERSION = "v10_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in scheduled-form v10 working copy, so resulting evidence is "
        "local-only build/probe evidence for the disjoint second-stripe "
        "consumer-buffer follow-up on top of the winning v7 transpose1 lane."
    ),
    "notes": [
        "Use this path to confirm that the scheduled-form v10 working copy can be consumed by the existing local schedule-preserving seam.",
        "The checked-in v10 file keeps v7's dc_0-sliced staged reuse intact and routes h_1 == 1 through a disjoint current buffer.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v10 TIR: "
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Do not board-benchmark this branch in its current state; the first "
            "local proof against the frozen v7 working copy failed and the seam "
            "mapping needs another redesign."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in scheduled-form v10 working copy is consumed only through the local post-db scheduled swap seam here.",
            "This v10 follow-up keeps v7's staged reuse intact while routing the second h_1 stripe through a disjoint consumer-facing current buffer.",

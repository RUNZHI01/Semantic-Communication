"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose1_add9 v6.

This module points the local schedule-preserving seam at a v6 working copy that
keeps the winning transpose1 v4 staging/reuse family intact while narrowing the
materialized data_dilate/data_pad producer window to one h_1 stripe at a time.
Each staged 34 x 10 stripe is then reused across all three output-channel
groups and both w_1 positions before the next stripe is prepared.
"""
...
WORKING_COPY_TIR_FILENAME = (
    f"{OPERATOR_NAME}_scheduled_form_candidate_v6_working_copy_tir.py"
)
WORKING_COPY_MANIFEST_FILENAME = "scheduled_form_candidate_v6_working_copy_manifest.json"
CANDIDATE_VERSION = "v6_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in scheduled-form v6 working copy, so resulting evidence is "
        "local-only build/probe evidence for a narrower transpose1 stripe-"
        "staging follow-up on top of the winning v4 locality lane."
    ),
    "notes": [
        "Use this path to confirm that the scheduled-form v6 working copy can be consumed by the existing local schedule-preserving seam.",
        "The checked-in v6 file keeps v4's stage-once staging/reuse family but narrows the producer window to one h_1 stripe at a time.",
        "That staged stripe is then reused across all three c_1 groups and both w_1 positions before advancing.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v6 TIR: "
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Take this exact v6 working copy through the existing local proof "
            "path, then benchmark the resulting swapped artifact on the board "
            "against the frozen v4 winner without mutating older transpose1 files."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in scheduled-form v6 working copy is consumed only through the local post-db scheduled swap seam here.",
            "This v6 follow-up narrows producer staging to one h_1 stripe while keeping the winning v4 materialized-buffer strategy and c_1-first consumer family intact.",

"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose1_add9 v9.

This module points the local schedule-preserving seam at a v9 working copy that
keeps the winning v7 stripe shape, dc_0 slice staging, and c_1-before-w_1
consumer order intact while adding an explicit 2-row consumer-facing data_pad
carry at the h_1 seam. The dropped v8 single-input-channel slice branch stays
closed here.
"""
...
WORKING_COPY_TIR_FILENAME = (
    f"{OPERATOR_NAME}_scheduled_form_candidate_v9_working_copy_tir.py"
)
WORKING_COPY_MANIFEST_FILENAME = "scheduled_form_candidate_v9_working_copy_manifest.json"
CANDIDATE_VERSION = "v9_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in scheduled-form v9 working copy, so resulting evidence is "
        "local-only build/probe evidence for the explicit two-row data_pad "
        "carry proof candidate on top of the winning v7 transpose1 lane."
    ),
    "notes": [
        "Use this path to confirm that the scheduled-form v9 working copy can be consumed by the existing local schedule-preserving seam.",
        "The checked-in v9 file keeps v7's dc_0-sliced staged reuse intact and adds an explicit consumer-facing 2-row data_pad carry at the h_1 seam.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v9 TIR: "
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Prove exact local correctness against the frozen v7 working copy, "
            "then benchmark the resulting swapped artifact on the board only if "
            "that local proof passes cleanly."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in scheduled-form v9 working copy is consumed only through the local post-db scheduled swap seam here.",
            "This v9 follow-up keeps v7's staged reuse intact while making the 2-row consumer-facing data_pad carry explicit at the h_1 seam.",

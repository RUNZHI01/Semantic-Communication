"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose1_add9 v3.

This module points the local schedule-preserving seam at a new v3 working copy
that applies the documented P3 path-A direct-stride-read idea on top of the
accepted v1/P2/P4 state without mutating the accepted baseline files.
"""
...
WORKING_COPY_MANIFEST_FILENAME = "scheduled_form_candidate_v3_working_copy_manifest.json"
CANDIDATE_VERSION = "v3_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in scheduled-form v3 working copy, so resulting evidence is "
        "local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the scheduled-form v3 working copy can be consumed by the existing local schedule-preserving seam.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v3 TIR: "
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in scheduled-form v3 working copy is consumed only through the local post-db scheduled swap seam here.",

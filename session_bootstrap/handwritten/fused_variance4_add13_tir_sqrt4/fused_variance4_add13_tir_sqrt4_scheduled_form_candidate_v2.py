"""Checked-in local post-db candidate wrapper for variance4 v2.

This module points the existing local schedule-preserving seam at a new v2
working copy that makes one conservative epilogue simplification on top of the
frozen v1 seed-aligned baseline without mutating the accepted local seam.
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v2 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v2 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v2 working copy is consumed only through the local post-db scheduled swap seam here.",

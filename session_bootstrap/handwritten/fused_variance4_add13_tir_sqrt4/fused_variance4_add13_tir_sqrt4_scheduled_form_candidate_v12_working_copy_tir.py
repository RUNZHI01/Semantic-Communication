# Editable scheduled-form candidate v12 working copy for fused_variance4_add13_tir_sqrt4.
#
# Derived from the checked-in scheduled-form v11 working copy.
#
# Contract:
# - local-only diagnostic working copy
# - keep the checked-in v1/v2/v3/v4/v5/v6/v6a/v7/v8/v9/v10/v11 candidates
...
#   intact
# - remove only the separate raw T.allocate local data handle while still
#   expressing the one-element local buffer directly at source level via
#   T.decl_buffer(..., scope="local")
# - keep the same float32 local store/load boundary, the same two reductions,
#   and the same output signature as v11

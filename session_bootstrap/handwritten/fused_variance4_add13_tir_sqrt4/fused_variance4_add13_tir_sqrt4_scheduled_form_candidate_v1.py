"""Checked-in local post-db candidate wrapper for variance4.
...
- it does not introduce a raw pre-compile handwritten lane for variance4
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v1 working copy, so resulting evidence "
        "is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v1 working copy can be consumed by the existing local post-db path.",
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Use run_variance4_post_db_local_build.py to consume this checked-in "
            "variance4 v1 working copy through the local post-db path."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v1 working copy is consumed only through the local post-db scheduled swap seam here.",

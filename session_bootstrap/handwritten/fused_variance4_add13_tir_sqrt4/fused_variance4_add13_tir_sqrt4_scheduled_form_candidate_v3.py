"""Checked-in local post-db candidate wrapper for variance4 v3.

This module points the existing local schedule-preserving seam at a new v3
working copy that removes one more small 1x12x1x1 stage on top of the checked-in
v2 candidate while keeping the frozen v1 baseline intact.
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v3 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v3 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v3 working copy is consumed only through the local post-db scheduled swap seam here.",

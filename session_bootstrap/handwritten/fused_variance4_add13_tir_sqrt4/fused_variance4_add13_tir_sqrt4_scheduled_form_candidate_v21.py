"""Checked-in local post-db candidate wrapper for variance4 v21.

This module points the existing local schedule-preserving seam at a new v21
working copy that retries the scalar mean handoff from v19 while restoring the
explicit local T_multiply_red storage used by the board-proven v18 branch.
"""
...
WORKING_COPY_TIR_FILENAME = (
    f"{OPERATOR_NAME}_scheduled_form_candidate_v21_working_copy_tir.py"
)
WORKING_COPY_MANIFEST_FILENAME = (
    "scheduled_form_candidate_v21_working_copy_manifest.json"
)
CANDIDATE_VERSION = "v21_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v21 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v21 working copy can be consumed by the existing local post-db path.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v21 TIR: "
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v21 working copy is consumed only through the local post-db scheduled swap seam here.",

"""Checked-in local post-db candidate wrapper for variance4 v9.

This module points the existing local schedule-preserving seam at a new v9
working copy that keeps the explicit one-element float32 local store/load
round-trip from v8 while removing only the `volatile_scope` attribute from the
local allocation.
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v9 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v9 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v9 working copy is consumed only through the local post-db scheduled swap seam here.",

# Editable scheduled-form candidate v9 working copy for fused_variance4_add13_tir_sqrt4.
#
# Derived from the checked-in scheduled-form v8 working copy.
#
# Contract:
# - local-only diagnostic working copy
# - keep the checked-in v1/v2/v3/v4/v5/v6/v6a/v7/v8 candidates intact; iterate
#   here for the next narrow handwritten variance4 simplification follow-up
# - do not treat this file as hook-facing or as performance evidence
#
# New candidate goal:
# - keep the explicit one-element float32 local store/load round-trip introduced
#   by v6a and preserved by v7/v8
# - remove only the volatile_scope attribute from that one-element local
#   allocation while keeping the local allocation, declared local buffer, and
#   store/load boundary intact

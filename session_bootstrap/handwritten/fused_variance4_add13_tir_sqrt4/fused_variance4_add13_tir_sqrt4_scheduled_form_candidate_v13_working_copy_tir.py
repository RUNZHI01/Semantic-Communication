# Editable scheduled-form candidate v13 working copy for fused_variance4_add13_tir_sqrt4.
#
# Derived from the checked-in scheduled-form v12 working copy.
#
# Contract:
# - local-only diagnostic working copy
# - keep the checked-in v1/v2/v3/v4/v5/v6/v6a/v7/v8/v9/v10/v11/v12 candidates
#   intact; iterate here for the next narrow handwritten variance4
#   simplification follow-up
# - do not treat this file as hook-facing or as performance evidence
#
# New candidate goal:
# - keep the v12 explicit `.data`-level volatile one-element local round-trip
#   intact
# - remove only the redundant hardcoded unit indices on lv335_red accesses in
#   the folded T_multiply_local expression by using the already-remapped
#   unit-extent axes v_ax2 and v_ax3 instead
# - keep the same float32 local store/load boundary, the same two reductions,
#   and the same output signature as v12
# - preserve the v6 full-size T_multiply removal, the v5 subtraction fold, the
#   v4 mean divide fold, the v3 variance-normalization divide fold, and the v2
#   epsilon add fused into sqrt
...
class Module:
    ...
    def fused_variance4_add13_tir_sqrt4(
        ...
        T_multiply_local = T.decl_buffer((1,), "float32", scope="local")

# Editable scheduled-form candidate v23 working copy for fused_variance4_add13_tir_sqrt4.
#
# Derived from the checked-in scheduled-form v18 working copy.
#
# Contract:
# - local-only diagnostic working copy
# - keep the checked-in v1/v2/v3/v4/v5/v6/v6a/v7/v8/v9/v10/v11/v12/v13/v14/v15/v16/v17/v18/v19/v20/v21/v22
...
# - keep the winning v18 handoff family intact: normalized-mean handoff,
#   centered-value T_subtract_local handoff, and local T_multiply_red storage
# - change only the tiny per-channel buffer shapes from `(1, 12, 1, 1)` to
#   `(12,)` so the codegen-visible indexing on the 12-channel reductions is
#   tightened without changing the arithmetic or reduction order
...
class Module:
    ...
    def fused_variance4_add13_tir_sqrt4(
        ...
        lv335_mean_local = T.alloc_buffer((T.int64(12),), "float32", scope="local")
        ...
        for ax0, ax1, ax2, ax3, k2, k3 in T.grid(
            ...
            with T.sblock("lv335_red"):
                ...
                T.writes(lv335_red[v_ax1])
                with T.init():
                    lv335_red[v_ax1] = T.float32(0.0)
                lv335_red[v_ax1] = lv335_red[v_ax1] + lv335[v_ax0, v_ax1, v_k2, v_k3]
        for ax0, ax1, ax2, ax3 in T.grid(T.int64(1), T.int64(12), T.int64(1), T.int64(1)):
            with T.sblock("lv335_mean_local"):
                v_ax0, v_ax1, v_ax2, v_ax3 = T.axis.remap("SSSS", [ax0, ax1, ax2, ax3])
                T.reads(lv335_red[v_ax1])
                T.writes(lv335_mean_local[v_ax1])
                lv335_mean_local[v_ax1] = lv335_red[v_ax1] / T.float32(65536.0)
        for ax0, ax1, ax2, ax3, k2, k3 in T.grid(
            T.int64(1), T.int64(12), T.int64(1), T.int64(1), T.int64(256), T.int64(256)
        ):
            with T.sblock("T_subtract_local"):
                ...
                T.reads(
                    ...
                    lv335_mean_local[v_ax1],
                )
                T.writes(T_subtract_local[0])
                T_subtract_local[0] = (
                    lv335[v_ax0, v_ax1, v_k2, v_k3] - lv335_mean_local[v_ax1]
                )
            ...
            with T.sblock("T_multiply_red"):
                ...
                T.writes(T_multiply_red[v_ax1])
                with T.init():
                    T_multiply_red[v_ax1] = T.float32(0.0)
                T_multiply_red[v_ax1] = T_multiply_red[v_ax1] + T_multiply_local[0]
        for i0, i1, i2, i3 in T.grid(T.int64(1), T.int64(12), T.int64(1), T.int64(1)):
            with T.sblock("compute"):
                v_i0, v_i1, v_i2, v_i3 = T.axis.remap("SSSS", [i0, i1, i2, i3])
                T.reads(T_multiply_red[v_i1])
                T.writes(compute_intermediate[v_i0, v_i1, v_i2, v_i3])
                compute_intermediate[v_i0, v_i1, v_i2, v_i3] = T.sqrt(
                    T_multiply_red[v_i1] / T.float32(65536.0)

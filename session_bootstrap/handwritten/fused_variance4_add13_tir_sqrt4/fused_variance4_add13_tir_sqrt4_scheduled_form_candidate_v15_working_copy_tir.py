# Editable scheduled-form candidate v15 working copy for fused_variance4_add13_tir_sqrt4.
#
# Derived from the checked-in scheduled-form v14 working copy.
#
# Contract:
# - local-only diagnostic working copy
# - keep the checked-in v1/v2/v3/v4/v5/v6/v6a/v7/v8/v9/v10/v11/v12/v13/v14
...
#
# New candidate goal:
# - keep the full v14 exact-preserving handle-free `.data`-volatile local
#   round-trip intact
# - make only the tiny lv335 reduction buffer explicitly `scope="local"` so
#   the hot second reduction reloads that per-channel mean from an explicit
#   local allocation rather than the default storage encoding
# - keep T_multiply_red storage, the same two reductions, and the same output
#   signature as v14

"""Checked-in local post-db candidate wrapper for variance4 v6.

This module points the existing local schedule-preserving seam at a new v6
working copy that removes the remaining full-size `T_multiply` stage on top of
the checked-in v5 candidate while keeping the frozen v1/v2/v3/v4/v5 states
intact.
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v6 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v6 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v6 working copy is consumed only through the local post-db scheduled swap seam here.",

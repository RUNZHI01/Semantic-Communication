"""Checked-in local post-db candidate wrapper for variance4 v10.

This module points the existing local schedule-preserving seam at a new v10
...
WORKING_COPY_MANIFEST_FILENAME = (
    "scheduled_form_candidate_v10_working_copy_manifest.json"
)
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v10 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v10 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v10 working copy is consumed only through the local post-db scheduled swap seam here.",

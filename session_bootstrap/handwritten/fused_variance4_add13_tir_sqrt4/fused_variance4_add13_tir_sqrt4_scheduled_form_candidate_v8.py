"""Checked-in local post-db candidate wrapper for variance4 v8.

This module points the existing local schedule-preserving seam at a new v8
working copy that keeps the exactness-preserving v7 volatile scalar round-trip
while removing the redundant explicit `scope="local"` from `T.decl_buffer(...)`
for the one-element local buffer declaration.
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v8 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v8 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v8 working copy is consumed only through the local post-db scheduled swap seam here.",

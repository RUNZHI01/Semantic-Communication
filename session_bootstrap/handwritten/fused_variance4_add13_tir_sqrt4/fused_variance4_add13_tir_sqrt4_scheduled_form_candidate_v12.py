"""Checked-in local post-db candidate wrapper for variance4 v12.

This module points the existing local schedule-preserving seam at a new v12
working copy that keeps the exact-preserving v11 `.data`-level volatility
encoding intact while removing the separate raw local allocation handle and
declaring the one-element local buffer directly.
"""
...
WORKING_COPY_TIR_FILENAME = (
    f"{OPERATOR_NAME}_scheduled_form_candidate_v12_working_copy_tir.py"
)
WORKING_COPY_MANIFEST_FILENAME = (
    "scheduled_form_candidate_v12_working_copy_manifest.json"
)
CANDIDATE_VERSION = "v12_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in variance4 scheduled-form v12 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the variance4 scheduled-form v12 working copy can be consumed by the existing local post-db path.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v12 TIR: "
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in variance4 scheduled-form v12 working copy is consumed only through the local post-db scheduled swap seam here.",

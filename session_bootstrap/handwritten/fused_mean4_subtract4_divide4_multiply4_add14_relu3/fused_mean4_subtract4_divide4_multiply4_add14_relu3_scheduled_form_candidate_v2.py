"""Checked-in local post-db candidate wrapper for mean4 v2.

This module points the local schedule-preserving seam at a new v2 working copy
that keeps the v1 reduction structure but replaces the full-frame epilogue
chain with tiny local handoff buffers.
"""
...
WORKING_COPY_MANIFEST_FILENAME = (
    "scheduled_form_candidate_v2_working_copy_manifest.json"
)
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in mean4 scheduled-form v2 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the mean4 scheduled-form v2 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in mean4 scheduled-form v2 working copy is consumed only through the local post-db scheduled swap seam here.",

"""Checked-in local post-db candidate wrapper for mean4 v3.

This module points the local schedule-preserving seam at a new v3 working copy
that performs a smallest operator-specific ablation after v2 regression.
"""
...
WORKING_COPY_MANIFEST_FILENAME = (
    "scheduled_form_candidate_v3_working_copy_manifest.json"
)
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in mean4 scheduled-form v3 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the mean4 scheduled-form v3 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in mean4 scheduled-form v3 working copy is consumed only through the local post-db scheduled swap seam here.",

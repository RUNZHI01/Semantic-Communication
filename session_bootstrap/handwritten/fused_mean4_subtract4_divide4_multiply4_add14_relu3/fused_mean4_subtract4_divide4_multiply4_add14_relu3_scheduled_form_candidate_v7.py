"""Checked-in local post-db candidate wrapper for mean4 v7.

This module points the local schedule-preserving seam at a new v7 working copy
that keeps the v5 affine epilogue but changes only the reduction side into a
4-lane partial-sum form.
"""
...
WORKING_COPY_MANIFEST_FILENAME = (
    "scheduled_form_candidate_v7_working_copy_manifest.json"
)
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in mean4 scheduled-form v7 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the mean4 scheduled-form v7 working copy can be consumed by the existing local post-db path.",
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in mean4 scheduled-form v7 working copy is consumed only through the local post-db scheduled swap seam here.",

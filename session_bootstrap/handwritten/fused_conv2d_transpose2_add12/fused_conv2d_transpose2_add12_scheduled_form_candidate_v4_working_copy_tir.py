# Editable scheduled-form candidate v4 working copy for fused_conv2d_transpose2_add12.
#
...
#
# First real v4 candidate goal:
# - keep the accepted v1 bias-fused compute path intact
# - keep data_dilate, data_pad, and kernel_transform materialized
# - keep the scheduled h/w tiling, reduction split, outer h-tile sweep, and
#   pragma_auto_unroll_max_step=32 intact
# - move data_dilate/data_pad staging inside w_0 so only the current 10 x 34
#   width window is prepared at a time instead of the full 10 x 258 strip
# - immediately reuse that staged width window across all four h_1 rows before
#   advancing to the next w_0 tile
...
class Module:
    @T.prim_func
    def fused_conv2d_transpose2_add12(
        lv332: T.Buffer((T.int64(1), T.int64(24), T.int64(128), T.int64(128)), "float32"),
        param_0: T.Buffer((T.int64(24), T.int64(12), T.int64(3), T.int64(3)), "float32"),
        lv334: T.Buffer((T.int64(1), T.int64(12), T.int64(1), T.int64(1)), "float32"),
        T_add_intermediate: T.Buffer((T.int64(1), T.int64(12), T.int64(256), T.int64(256)), "float32"),
    ):
        ...
        for b_0_c_0_h_0_fused_fused_fused in T.parallel(
            T.int64(32),
            annotations={"pragma_auto_unroll_max_step": 32, "pragma_unroll_explicit": 1},
        ):
            for w_0 in T.serial(T.int64(8)):
                for ax0, ax1, ax2 in T.grid(T.int64(1), T.int64(24), T.int64(10)):
                    for ax0_1, ax1_1, ax2_1 in T.grid(T.int64(1), T.int64(1), T.int64(1)):
                        for ax3 in T.vectorized(T.int64(34)):
                            with T.sblock("data_dilate"):
                                v_i0 = T.axis.spatial(T.int64(1), ax0_1)
                                v_i1 = T.axis.spatial(T.int64(24), ax1 + ax1_1)
                                v_i2 = T.axis.spatial(
                                    T.int64(255),
                                    b_0_c_0_h_0_fused_fused_fused * T.int64(8)
                                    + ax2
                                    + T.int64(-1)
                                    + ax2_1,
                                )
                                v_i3 = T.axis.spatial(
                                    T.int64(255), w_0 * T.int64(32) + ax3 + T.int64(-1)
                                )
                                T.where(
                                    T.int64(1)
                                    <= b_0_c_0_h_0_fused_fused_fused * T.int64(8) + ax2
                                    and b_0_c_0_h_0_fused_fused_fused * T.int64(8) + ax2
                                    < T.int64(256)
                                    and T.int64(1) <= w_0 * T.int64(32) + ax3
                                    and w_0 * T.int64(32) + ax3 < T.int64(256)
                                )
                                T.reads(
                                    lv332[v_i0, v_i1, v_i2 // T.int64(2), v_i3 // T.int64(2)]
                                )
                    ...
                    for (
                        ...
                        for w_3_fused_init in T.vectorized(T.int64(8)):
                            with T.sblock("compute_init"):
                                ...
                                v_c = T.axis.spatial(
                                    T.int64(12),
                                    c_1 * T.int64(12) + c_2_init * T.int64(12) + c_3_init,
                                )
                                v_h = T.axis.spatial(
                                    T.int64(256),
                                    b_0_c_0_h_0_fused_fused_fused * T.int64(8)
                                ...
                                T_add_intermediate[v_b, v_c, v_h, v_w] = lv334[
                    ...
                    for (
                        ...
                        for w_3_fused in T.vectorized(T.int64(8)):
                            with T.sblock("compute_update"):
                                ...
                                v_c = T.axis.spatial(
                                    T.int64(12), c_1 * T.int64(12) + c_2 * T.int64(12) + c_3
                                )
                                v_h = T.axis.spatial(
                                    T.int64(256),
                                    b_0_c_0_h_0_fused_fused_fused * T.int64(8)

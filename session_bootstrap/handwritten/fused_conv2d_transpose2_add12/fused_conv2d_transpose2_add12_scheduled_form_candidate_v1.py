"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose2_add12.
...
- it does not introduce a raw pre-compile handwritten lane for transpose2
"""
...
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in transpose2 scheduled-form v1 working copy, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the transpose2 scheduled-form v1 working copy can be consumed by the existing local schedule-preserving seam.",
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Use run_transpose2_post_db_local_build.py to consume this checked-in "
            "transpose2 v1 working copy through the local post-db path."
        ),
    }
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in transpose2 scheduled-form v1 working copy is consumed only through the local post-db scheduled swap seam here.",

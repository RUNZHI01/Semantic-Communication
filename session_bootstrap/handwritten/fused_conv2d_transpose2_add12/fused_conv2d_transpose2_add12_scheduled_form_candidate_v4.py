"""Checked-in local post-db candidate wrapper for fused_conv2d_transpose2_add12 v4.

This module points the local schedule-preserving seam at the first real v4
working-copy candidate. The edit keeps the accepted transpose2 v1 arithmetic
path intact while staging only the current `w_0`-local `10 x 34` padded window
instead of the full-width `10 x 258` strip.
"""
...
WORKING_COPY_MANIFEST_FILENAME = "scheduled_form_candidate_v4_working_copy_manifest.json"
CANDIDATE_VERSION = "v4_working_copy"
DEFAULT_EVALUATION_CONTRACT = {
    ...
    "reason": (
        ...
        "checked-in transpose2 scheduled-form v4 candidate, so resulting "
        "evidence is local-only build/probe evidence."
    ),
    "notes": [
        "Use this path to confirm that the transpose2 scheduled-form v4 candidate can be consumed by the existing local schedule-preserving seam.",
...
def _load_working_copy_manifest() -> dict[str, Any]:
    ...
    if expected_sha is not None and actual_sha != expected_sha:
        raise ValueError(
            "working copy manifest SHA does not match the checked-in v4 TIR: "
...
def describe_placeholder(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": (
            "Benchmark the exported v4 width-window-staging artifact on the "
...
def build_manual_impl(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            "The checked-in transpose2 scheduled-form v4 candidate is consumed only through the local post-db scheduled swap seam here.",

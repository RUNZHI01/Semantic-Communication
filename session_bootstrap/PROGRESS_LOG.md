...
- 最后更新：2026-03-14 19:24 +0800（补记 2026-03-14 基于 `release_v1.4.0` 的兼容 patch 已在飞腾板上完成一次真实 wrapper-backed board smoke：当前 live `/lib/firmware/openamp_core0.elf` 仍为 `JOB_ACK` 版固件，size `1640048`、SHA `98ab501c...647e`；在板上 wrapper 通过 hook 调 bridge 发出真实 `JOB_REQ`（`job_id=9205`）后，已收到 `source=firmware_job_ack` 的 `JOB_ACK(ALLOW)`，`fault_code=0`、`guard_state=2`，随后 wrapper 真实放行 runner 并收到了 `JOB_DONE(success)`。这标志着 OpenAMP 已从最小作业授权进一步推进到 wrapper × bridge × firmware × board 的真实 admission gate 已串接成功。）

...
- 最后更新：2026-03-18 04:35 +0800（本轮工作重心已经从“补文档入口”切回“把 demo 真正跑起来”，随后又进一步转向 **big.LITTLE 异构大小核方向** 的夜间 repo 侧准备。现状已经明显前进：**current live 已在最新代码下真实跑通 `300/300`**，并完成 `STATUS_REQ(READY) -> JOB_REQ(ALLOW) -> HEARTBEAT_ACK -> JOB_DONE(success)` 的整条控制面闭环；其间也已修复 current trusted SHA 错配（`bc9d836`）、torch sidecar fallback/runner 导入问题（`f17c665` / `ab01b5f` / `6b9a8a7`）。另一方面，**baseline 已不再停留在归档 reference 面板，而是已接成真实 PyTorch live 执行路径**（`f94fee1`），但最新终验表明板端仍会对其返回 `JOB_ACK(DENY, ARTIFACT_SHA_MISMATCH)`；也就是说，当前剩余主 blocker 已从 demo 接线切到固件/控制面对 PyTorch generator checkpoint 工件语义的接受规则。与此同时，big.LITTLE 这条线也已从纸面方案推进成 repo 内可执行脚手架，并完成了最后一圈首跑收口：除 `big_little_pipeline.py` / `run_big_little_pipeline.sh` / `run_big_little_compare.sh` / 两份 env 模板 / runbook（`cb1323f`）外，又补齐了本地 mock compare 验证修复（`0650fbb`）、只读 topology helper（`f9e0e0b`）、首轮大小核建议落盘（`cacd912`）、一键首跑入口（`b56030d`）、env 自动回填 helper（`b730020`）、拓扑超时与 runtime env 隔离（`e3ef55e` / `ddf29ec`）、runbook/README/artifact registry 入口联通（`3e3f024` / `e9fa7ed` / `7055d3e`）、一键首跑脚本的 local/mock 全链路通过（`3d8b5c3`）、明早短交接页与主入口回链（`6bdaafe`），以及短交接页与完整 runbook 的双向互链（`8461fc0`）。这说明明早真正需要上板做的已经不再是设计或调脚本，而只是按一键入口完成 topology re-check、pipeline current 实跑和 serial vs pipeline 吞吐对比；若从时间线入口接手，先看 `session_bootstrap/reports/big_little_overnight_handoff_20260318.md`，再按需跳到 `session_bootstrap/runbooks/big_little_pipeline_runbook_2026-03-18.md`。）

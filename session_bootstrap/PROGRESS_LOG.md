...
- 最后更新：2026-03-15 02:00 +0800（P1 的三类正式 FIT 现已全部拿到真实板级结论：`FIT-01` 错误 SHA = PASS、`FIT-02` 输入契约破坏 = PASS、`FIT-03` 心跳超时 / watchdog = **FAIL（缺口已被真实确认）**。最新 `FIT-03` 在 fresh boot 后按干净 bring-up 路径 `set_env.sh -> sudo rpmsg-demo` 恢复 `/dev/rpmsg0`，然后执行 `pre STATUS_REQ -> JOB_REQ(ALLOW) -> HEARTBEAT_ACK(heartbeat_ok=1) -> 停发 heartbeat 5s -> STATUS_REQ`；结果显示固件在 5 秒无 heartbeat 后仍停留在 `guard_state=JOB_ACTIVE / active_job_id=9303 / last_fault_code=0 / heartbeat_ok=1 / total_fault_count=0`，并不会自动转成 `HEARTBEAT_TIMEOUT(F003)` 或自动 stop。最后只能通过显式 `SAFE_STOP` 做清理，清理后状态回到 `READY / active_job_id=0 / last_fault_code=MANUAL_SAFE_STOP(10) / total_fault_count=1`。这说明当前控制面已经拥有两类正式拒绝证据，但自动 watchdog 语义仍是下一步必须补齐的真实缺口。）
...
- **当前 live firmware 的主缺口已收敛到 heartbeat watchdog 自动语义**：
  - `FIT-01` 与 `FIT-02` 已经证明 admission gate 的两类拒绝（错误 SHA / 非法参数）都能在固件边界直接触发真实 `DENY`；
  - `FIT-03` 则证明当前 live firmware 在已接收一次合法 heartbeat 后，至少在本轮测试的 `5s` 无 heartbeat 窗口内不会自动转成 `HEARTBEAT_TIMEOUT(F003)`、也不会自动 stop；
  - 因此下一阶段最值钱的工作不再是继续补更多拒绝类 FIT，而是：先把 firmware 侧 heartbeat timeout / watchdog 逻辑真正接上，再回头把 FIT-03 从 FAIL 改成 PASS。

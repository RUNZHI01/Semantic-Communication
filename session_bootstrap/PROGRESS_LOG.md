...
- 最后更新：2026-03-15 01:48 +0800（P1 已连续拿到两类正式真机异常注入证据：`FIT-01` 错误 SHA 与 `FIT-02` 输入契约破坏都已在飞腾派板上通过真实 `RPMsg` / firmware `JOB_ACK(DENY)` 收证。最新 `FIT-02` 使用合法 trusted SHA、但故意发送非法 `expected_outputs=2`；其 pre-status 为干净 `READY/active_job_id=0/last_fault=NONE/total_fault_count=0`，真实 `JOB_REQ` 收到 `JOB_ACK(DENY)` 且 `fault_code=9 / ILLEGAL_PARAM_RANGE`、`guard_state=READY`，wrapper 收敛为 `denied_by_control_hook` 且 runner 未启动，post-status 维持 `READY/active_job_id=0` 并把 `last_fault=ILLEGAL_PARAM_RANGE`、`total_fault_count=1` 稳定落盘。当前 P1 只剩 `FIT-03` 心跳超时 / watchdog 语义待补，这意味着控制面已经从“最小能工作”进入到“带正式风险拒绝证据”的阶段。）

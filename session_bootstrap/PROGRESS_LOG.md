...
- 最后更新：2026-03-31 13:50 +0800（本轮工作重心已从“judge evidence / profiling 补证”进一步切到 **`fused_conv2d_transpose1_add9` 的 handwritten / scheduled-form 局部深挖**。截至目前，这条线已经不再停留在流程脚手架：首先确认了旧 raw pre-compile handwritten `v0` 虽然能打通 hook，但 safe runtime payload 明显回退（best staging `5bd14b9f...` 为 `159.943 ms` payload，而 handwritten `v0` 只有 `655.693 ms`，因此 `v0` 已正式判 `drop / not for reprobe`）；随后围绕该回归原因完成了 schedule-preserving 本地验证链的搭建：从 seam 设计、DB query probe、post-db scheduled swap、本地 build/export、scaffold/bookkeeping/snapshot、one-shot build+sync wrapper，一直到最新的 checked-in snapshot 全部打通。更重要的是，这条线现在已经进入真正的 operator-side 迭代：仓库内已新增 checked-in `post_db_scheduled_reference_seed`、`scheduled-form v1 working copy`，并在这份 working copy 上落下第一版真实 `v1` 改写（bias 融入 scheduled compute init/update，移除 `compute_intermediate` 与尾部 `T_add`）。最新状态是：`v1` 已经可以通过现有 local schedule-preserving path 被真实消费，`swap_succeeded=true`，并成功导出本地产物 SHA `4f0986e4806bece9801ab38b4ec121870406476c3d9a1c870bbc0453e18ef2fc`。但这里要特别明确：`159.943 ms` 仍然只是 **safe runtime payload 时间**，不是端到端 reconstruction 时间；而当前 `v1` 只有本地 build / swap / artifact 证据，还没有可发布的 runtime/performance 结论。因此这轮工作的最诚实结论是：`transpose1` 已从“会误导性能判断的 raw handwritten 路径”推进到“可以本地 schedule-preserving 验证的 scheduled-form v1 路径”，但**还没有证据证明它已经加速**。下一步应优先补一份 repo-local `v1` evidence / compare note，再决定是否值得进入更强验证。）

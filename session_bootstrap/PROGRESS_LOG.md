...
- 最后更新：2026-03-14 21:45 +0800（补记 2026-03-14 `SAFE_STOP` 最小真机闭环已验证成功：当前 live `/lib/firmware/openamp_core0.elf` 为 `SAFE_STOP` 版固件，size `1647272`、SHA `3e7512fe...0424`；在真机上完成 `JOB_REQ(ALLOW)`、`HEARTBEAT_ACK(heartbeat_ok=1)` 之后，再发 `SAFE_STOP` 已收到 post-stop `STATUS_RESP`，其状态为 `guard_state=READY`、`active_job_id=0`、`last_fault_code=MANUAL_SAFE_STOP(10)`、`heartbeat_ok=0`、`total_fault_count=1`，而 follow-up `STATUS_REQ` 继续保持同样状态。这说明 OpenAMP 已从最小运行中监护进一步推进到最小安全停止可用阶段。）

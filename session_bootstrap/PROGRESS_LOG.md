...
- 最后更新：2026-03-30 18:47 +0800（本轮工作重心已经从“demo / admission 主线”切到 **评委高频追问补证**，并已基本收口。当前最新 judge-facing 默认入口应统一为 `session_bootstrap/reports/judge_evidence_pack_20260330_current_chunk4_lpips_full_profiled.md`：其中 quality 已升级到 **LPIPS full**（`PyTorch vs TVM baseline/current` 与 `TVM baseline vs current` 三组全齐），multi-SNR 已补齐 trusted current `chunk4` 的 `SNR=1/4/7/10/13` 五点真实 current latency + quality 曲线，resource profile 已切到 latest chunk4 版本，技术文档正文 / 答辩附录 / 速查卡也都已统一到最新口径。更关键的是，先前被视为剩余唯一技术 blocker 的 remote runtime per-op profiling 也已经打通：问题根因是 VM 实例化路径没有启用 `profile=True`，导致底层 runtime module 未导出 profiling 符号；修复后，`session_bootstrap/reports/profiling_judge_retry_parse_20260330_184026.md` 已拿到首个 sample 级 remote runtime profile，而 `session_bootstrap/reports/profiling_judge_multi_20260330_184658.md` 又进一步把它扩到 `3 samples`，当前稳定落在前两位的 runtime hotspot candidates 为 `fused_conv2d_transpose2_add12` 与 `fused_conv2d_transpose1_add9`。因此，当前 judge-evidence 主线已经从“补表补图”推进到“质量 / 资源 / SNR / runtime profiling 四条线全部有证据”，剩余工作不再是补核心缺口，而更多是是否继续扩大 profiling 样本量、或回到更后置的性能深化路线，例如 hotspot 定向深搜、手写 TIR + NEON、TVM 量化、跨框架对比与 big.LITTLE 进一步优化。）

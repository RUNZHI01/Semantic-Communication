...
- 最后更新：2026-03-18 09:36 +0800（本轮工作重心已经从“补文档入口”切回“把 demo 真正跑起来”，随后又进一步转向 **big.LITTLE 异构大小核方向** 的夜间 repo 侧准备。现状已经明显前进：**current live 已在最新代码下真实跑通 `300/300`**，并完成 `STATUS_REQ(READY) -> JOB_REQ(ALLOW) -> HEARTBEAT_ACK -> JOB_DONE(success)` 的整条控制面闭环；其间也已修复 current trusted SHA 错配（`bc9d836`）、torch sidecar fallback/runner 导入问题（`f17c665` / `ab01b5f` / `6b9a8a7`）。另一方面，**baseline 已不再停留在归档 reference 面板，而是已接成真实 PyTorch live 执行路径**（`f94fee1`），但最新终验表明板端仍会对其返回 `JOB_ACK(DENY, ARTIFACT_SHA_MISMATCH)`；也就是说，当前剩余主 blocker 已从 demo 接线切到固件/控制面对 PyTorch generator checkpoint 工件语义的接受规则。与此同时，big.LITTLE 这条线也已经不再停留在“明早可跑”的准备态，而是已完成**首轮真机 pipeline + 两轮 serial-vs-pipeline compare + resource profiling + 关键证据入库 + 工作区降噪 + post-run 文案归一化**：中途虽然先后暴露出 topology suggestion 文件格式、一键 wrapper 变量注入、远端 current artifact 漂移、torch sidecar 注入、5 秒 join 过早终止 worker，以及 resource-profile tool probe 误判等多层 blocker，但都已被逐个定位并修掉（`69e7644` / `0c3b548` / `0633789` / `6730224` / `bebc0a0`，外加远端 current `.so` 恢复到 `6f236b07...`）。现在这条线已经同时具备：首轮真机 compare `+36.937%`、第二轮复跑 `+36.54%`、真机 profiling 资源证据、一页最终摘要 `session_bootstrap/reports/big_little_real_run_summary_20260318.md`、已经被 git 正式纳入的关键实证产物（`55bf447` / `0dec83e`），避免 future runtime env 副本刷屏的 `.gitignore` 收尾（`332d4cd`），以及把旧“明早/首跑前”入口统一重写成历史参考口径的后处理（`4f56edc`）。但这里要特别注意口径：这组 `+36%` 结论成立的前提是“相对同轮 serial current 基线”，**不是**说它已经超过历史最佳 current 端到端 `230.339 ms/image`；按总 wall time 折算，本轮 big.LITTLE pipeline 约 `253~254 ms/image`，仍比那份历史最佳慢约 `9.9%~10.5%`。因此这条线当前更准确的定位应是“已证明异构流水线在同轮 current serial 对照下有效”，而不是“已经刷新 current 绝对最快记录”；若从时间线入口接手，优先看 `session_bootstrap/reports/big_little_real_run_summary_20260318.md`，再按需下钻到 compare / profiling / runbook。）

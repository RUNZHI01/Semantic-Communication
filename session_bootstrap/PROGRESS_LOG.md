...
- 最后更新：2026-03-15 01:28 +0800（P1 `FIT-01` 已从“blocked 准备态”推进到真机成功：先由 Codex 子沙箱补齐了 `session_bootstrap/scripts/run_openamp_fit_wrong_sha.py` 与结构化 blocked bundle `session_bootstrap/reports/openamp_wrong_sha_fit_20260315_010828/`，随后切回主会话 `exec` 直接用仓库内 `ssh_with_password.sh` 连上飞腾派 `100.121.87.73:22`，在远端 `/tmp/openamp_wrong_sha_fit/project` 上执行 `pre STATUS_REQ -> wrapper wrong-SHA JOB_REQ -> post STATUS_REQ`。真实结果已落入 `session_bootstrap/reports/openamp_wrong_sha_fit_20260315_012403/`：pre-status 为干净 `READY/active_job_id=0/last_fault=NONE/total_fault_count=0`；wrong-SHA `JOB_REQ` 收到真实 `JOB_ACK(DENY)`，其 `fault_code=1 / ARTIFACT_SHA_MISMATCH`、`guard_state=READY`；wrapper 收敛为 `denied_by_control_hook` 且 runner 未启动；post-status 维持 `READY/active_job_id=0`，同时 `last_fault=ARTIFACT_SHA_MISMATCH`、`total_fault_count=1`。这标志着 OpenAMP P1 的第一个正式异常注入用例已经拥有真实板级证据，而不仅是模板或 blocked 预案。）

...
- 最后更新：2026-03-18 05:16 +0800（本轮工作重心已经从“补文档入口”切回“把 demo 真正跑起来”，随后又进一步转向 **big.LITTLE 异构大小核方向** 的夜间 repo 侧准备。现状已经明显前进：**current live 已在最新代码下真实跑通 `300/300`**，并完成 `STATUS_REQ(READY) -> JOB_REQ(ALLOW) -> HEARTBEAT_ACK -> JOB_DONE(success)` 的整条控制面闭环；其间也已修复 current trusted SHA 错配（`bc9d836`）、torch sidecar fallback/runner 导入问题（`f17c665` / `ab01b5f` / `6b9a8a7`）。另一方面，**baseline 已不再停留在归档 reference 面板，而是已接成真实 PyTorch live 执行路径**（`f94fee1`），但最新终验表明板端仍会对其返回 `JOB_ACK(DENY, ARTIFACT_SHA_MISMATCH)`；也就是说，当前剩余主 blocker 已从 demo 接线切到固件/控制面对 PyTorch generator checkpoint 工件语义的接受规则。与此同时，big.LITTLE 这条线也已经不再停留在“明早可跑”的准备态，而是已完成**首轮真机 pipeline + serial-vs-pipeline compare**：中途虽然先后暴露出 topology suggestion 文件格式、一键 wrapper 变量注入、远端 current artifact 漂移、torch sidecar 注入以及 5 秒 join 过早终止 worker 等多层 blocker，但都已被逐个定位并修掉（`69e7644` / `0c3b548` / `0633789`，外加远端 current `.so` 恢复到 `6f236b07...`）。最终首轮真机结果已落盘：serial `2.886 images/s`、pipeline `3.952 images/s`、吞吐提升 `36.937%`，且真机 pipeline 本体 `processed_count=300`、`artifact_sha256_match=true`、绑定为 `big=[2]`、`little=[0,1]`。因此这条线当前已经从“待验证方案”升级成“已有首轮真机证据、可继续复跑/扩展 profiling 的工作基线”；若从时间线入口接手，优先看 `session_bootstrap/reports/big_little_compare_20260318_051326.md`，再按需回看 `session_bootstrap/reports/big_little_overnight_handoff_20260318.md` 与 `session_bootstrap/runbooks/big_little_pipeline_runbook_2026-03-18.md`。）

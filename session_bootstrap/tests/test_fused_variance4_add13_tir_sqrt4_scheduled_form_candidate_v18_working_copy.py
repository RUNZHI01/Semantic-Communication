...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v18_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v18_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV18WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_stages_centered_value_once(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("New candidate goal", text)
        self.assertIn('lv335_red = T.alloc_buffer(', text)
        self.assertIn('lv335_mean_local = T.alloc_buffer(', text)
        self.assertIn(
        ...
        self.assertIn('with T.sblock("lv335_mean_local")', text)
        self.assertIn('with T.sblock("T_subtract_local")', text)
        self.assertIn("T.writes(T_subtract_local[0])", text)
        self.assertIn("T_multiply_local[0] = T_subtract_local[0] * T_subtract_local[0]", text)
        self.assertIn("lv335_red[v_ax0, v_ax1, v_ax2, v_ax3] / T.float32(65536.0)", text)
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v18_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_centered_value_handoff_change(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v18_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v18_stage_centered_value_once_keep_v17_mean_handoff",
        )
        self.assertIn(
            "add a tiny one-element T_subtract_local buffer",

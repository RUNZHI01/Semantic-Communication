...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v8_working_copy_tir.py"
)
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV8WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_v7_boundary_but_drops_redundant_decl_buffer_scope(
        self,
    ) -> None:
        ...
        self.assertIn("T.attr(T_multiply_local_data, \"volatile_scope\", 1)", text)
        self.assertIn(
            "T_multiply_local = T.decl_buffer((1,), \"float32\", data=T_multiply_local_data)",
            text,
        )
        self.assertNotIn("data=T_multiply_local_data, scope=\"local\"", text)
    ...
    def test_manifest_records_the_decl_buffer_scope_removal_edit(self) -> None:
        ...
        self.assertEqual(
            ...
            "v8_drop_redundant_decl_buffer_local_scope_keep_v7_boundary",
        )
        self.assertIn(
            "removing the redundant explicit scope=\"local\"",

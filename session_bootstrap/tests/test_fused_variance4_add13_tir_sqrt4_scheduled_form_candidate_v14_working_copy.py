...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v14_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v14_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV14WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_switches_the_one_element_local_to_alloc_buffer(self) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v14_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_alloc_buffer_local_encoding(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v14_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v14_switch_one_element_local_to_alloc_buffer_keep_v13_roundtrip",
        )
        self.assertIn(
            "replace only the one-element local declaration-level encoding",

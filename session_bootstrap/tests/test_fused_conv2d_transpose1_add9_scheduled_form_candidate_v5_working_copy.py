...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_conv2d_transpose1_add9_scheduled_form_candidate_v5_working_copy_tir.py"
)
...
class FusedConv2dTranspose1Add9ScheduledFormCandidateV5WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_the_v5_edit_local_and_isolated(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("Accepted baseline carried forward into this v5 scaffold", text)
        self.assertIn("First real v5 locality/schedule edit", text)
        ...
        self.assertLess(
            ...
            text.index("for h_1, w_1 in T.grid"),
        )
        self.assertLess(
            text.index("for h_1, w_1 in T.grid"),
        ...
        self.assertIn(
            "consume each staged spatial subtile across all three output-channel groups",
            text,
        )
    ...
    def test_manifest_records_the_current_checked_in_v5_scaffold_state(self) -> None:
        ...
        self.assertEqual(
            ...
            "v5_h1w1_consumer_outer_on_top_of_v4_applied",
        )
        self.assertIn(
            "change only the consumer loop placement from c_1 -> h_1/w_1 to h_1/w_1 -> c_1",
        ...
        self.assertEqual(payload["accepted_baseline"]["remote_median_ms"], 158.621)

...
MODULE_PATH = (
    HANDWRITTEN_DIR / "fused_conv2d_transpose2_add12_scheduled_form_candidate_v4.py"
)
WORKING_COPY_TIR_PATH = (
    HANDWRITTEN_DIR
    / "fused_conv2d_transpose2_add12_scheduled_form_candidate_v4_working_copy_tir.py"
)

spec = importlib.util.spec_from_file_location(
    "fused_conv2d_transpose2_add12_scheduled_form_candidate_v4",
    MODULE_PATH,
)
...
seam_spec = importlib.util.spec_from_file_location(
    "probe_transpose1_schedule_preserving_seam_transpose2_v4_test",
    SEAM_PROBE_PATH,
)
...
class FusedConv2dTranspose2Add12ScheduledFormCandidateV4Test(unittest.TestCase):
    def test_wrapper_reports_local_only_post_db_contract(self) -> None:
        ...
        self.assertEqual(
            ...
            "v4_w0_window_data_stage_applied",
        )
        self.assertIn(
            "move data_dilate/data_pad staging inside w_0",
    ...
    def test_seam_probe_can_load_the_v4_wrapper(self) -> None:
                ...
                MODULE_PATH,
                {
                    "legalized_fused_tir": {
                        "tasks": [
                            {
                                "rank": 17,
                                "task_name": "fused_conv2d_transpose2_add12",
                                "prim_funcs": ["fused_conv2d_transpose2_add12"],
                            }
                        ]
                    }
                },
        ...
        self.assertEqual(payload["metadata"]["operator"], "fused_conv2d_transpose2_add12")
        self.assertEqual(payload["source_path"], WORKING_COPY_TIR_PATH.resolve())
        self.assertEqual(payload["source_func_name"], "fused_conv2d_transpose2_add12")

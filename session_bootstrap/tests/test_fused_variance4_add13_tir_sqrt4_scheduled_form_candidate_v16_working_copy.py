...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v16_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v16_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV16WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_only_local_scopes_t_multiply_red(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("New candidate goal", text)
        self.assertIn('lv335_red = T.alloc_buffer(', text)
        self.assertIn('T_multiply_red = T.alloc_buffer(', text)
        ...
        self.assertIn("T_multiply_red[v_ax0, v_ax1, v_ax2, v_ax3]", text)
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v16_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_t_multiply_red_local_scope_change(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v16_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v16_make_t_multiply_red_explicit_local_scope_keep_v15_storage_chain",
        )
        self.assertIn(
            "make only T_multiply_red explicitly T.alloc_buffer",

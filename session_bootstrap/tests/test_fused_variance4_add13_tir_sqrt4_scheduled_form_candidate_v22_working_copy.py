...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v22_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v22_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV22WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_retimes_mean_handoff_without_scalarizing_it(self) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v22_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_per_channel_retiming_change(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v22_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v22_per_channel_mean_retimed_consume",
        )
        self.assertIn(
            "retime the second phase so each per-channel mean is produced and then immediately consumed",

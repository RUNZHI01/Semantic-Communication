...
class FusedConv2dTranspose2Add12ScheduledFormCandidateV2WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_the_p1_edit_local_and_isolated(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("New candidate goal", text)
        self.assertIn("data_dilate_pad = T.alloc_buffer", text)
        self.assertIn('with T.sblock("data_dilate_pad")', text)
    ...
    def test_manifest_records_the_current_checked_in_v2_edit(self) -> None:
        ...
        self.assertIn(
            "replace the separate materialized data_dilate",

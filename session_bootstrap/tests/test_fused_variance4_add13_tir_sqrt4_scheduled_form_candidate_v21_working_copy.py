...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v21_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v21_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV21WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_retries_scalar_mean_handoff_with_local_reduce(self) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v21_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_retry_scope_isolation_change(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v21_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v21_scalar_mean_handoff_restore_local_reduce",
        )
        self.assertIn(
            "retry the scalar normalized-mean handoff shape from v19 but restore T_multiply_red to scope=\"local\"",

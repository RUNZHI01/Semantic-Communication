...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV3WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_the_second_edit_local_and_isolated(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("New candidate goal", text)
        self.assertNotIn("T_divide_intermediate = T.alloc_buffer", text)
        self.assertNotIn('with T.sblock("T_divide_1")', text)
    ...
    def test_manifest_records_the_current_checked_in_v3_edit(self) -> None:
        ...
        self.assertIn(
            "remove the standalone T_divide_intermediate buffer/stage",

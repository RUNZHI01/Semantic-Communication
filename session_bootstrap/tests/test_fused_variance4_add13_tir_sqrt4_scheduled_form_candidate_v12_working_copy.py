...
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v12_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV12WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_removes_raw_allocate_handle_but_keeps_data_volatility(
        self,
    ) -> None:
        ...
        self.assertNotIn("T_multiply_local_data = T.allocate([1], \"float32\", \"local\")", text)
        self.assertIn(
            "T_multiply_local = T.decl_buffer((1,), \"float32\", scope=\"local\")",
            text,
        )
        self.assertNotIn("data=T_multiply_local_data", text)
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v12_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_raw_allocate_handle_removal(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v12_working_copy",
        )
        ...
        self.assertIn(
            "remove only the separate raw T.allocate local data handle",

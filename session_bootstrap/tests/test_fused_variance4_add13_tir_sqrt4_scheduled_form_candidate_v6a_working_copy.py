...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v6a_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v6a_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV6aWorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_v6_simplification_but_adds_a_volatile_scalar_roundtrip(
        self,
    ) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v6a_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_exactness_recovery_edit(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v6a_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v6a_volatile_scalar_roundtrip_restores_exact_equality_on_top_of_v6",
        )
        self.assertIn(
            "one-element volatile local round-trip",

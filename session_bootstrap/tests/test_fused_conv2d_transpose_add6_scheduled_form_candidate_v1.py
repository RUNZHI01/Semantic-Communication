...
MODULE_PATH = (
    HANDWRITTEN_DIR / "fused_conv2d_transpose_add6_scheduled_form_candidate_v1.py"
)
...
spec = importlib.util.spec_from_file_location(
    "fused_conv2d_transpose_add6_scheduled_form_candidate_v1",
    MODULE_PATH,
)
...
seam_spec = importlib.util.spec_from_file_location(
    "probe_transpose1_schedule_preserving_seam_transpose_add6_v1_test",
    SEAM_PROBE_PATH,
)
...
class FusedConv2dTransposeAdd6ScheduledFormCandidateV1Test(unittest.TestCase):
    def test_wrapper_reports_local_only_post_db_contract(self) -> None:
        metadata = module.describe_placeholder()
        self.assertEqual(metadata["operator"], "fused_conv2d_transpose_add6")
    ...
    def test_seam_probe_can_load_the_v1_wrapper(self) -> None:
                ...
                MODULE_PATH,
                {
                    "legalized_fused_tir": {
                        "tasks": [
                            {
                                "rank": 18,
                                "task_name": "fused_conv2d_transpose_add6",
                                "prim_funcs": ["fused_conv2d_transpose_add6"],
                            }
                        ]
                    }
                },
        ...
        self.assertEqual(payload["metadata"]["operator"], "fused_conv2d_transpose_add6")
        self.assertEqual(payload["source_path"], WORKING_COPY_TIR_PATH.resolve())
        self.assertEqual(payload["source_func_name"], "fused_conv2d_transpose_add6")

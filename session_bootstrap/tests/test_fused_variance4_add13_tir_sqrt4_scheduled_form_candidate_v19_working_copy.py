...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_variance4_add13_tir_sqrt4_scheduled_form_candidate_v19_working_copy_tir.py"
)
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v19_working_copy_manifest.json"
...
class FusedVariance4Add13TirSqrt4ScheduledFormCandidateV19WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_tightens_mean_handoff_to_scalar(self) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v19_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_scalar_mean_handoff_change(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_variance4_add13_tir_sqrt4")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v19_working_copy",
        )
        ...
        self.assertEqual(
            ...
            "v19_tighten_mean_handoff_to_scalar_keep_v18_centered_value_reuse",
        )
        self.assertIn(
            "tighten the normalized-mean handoff itself to a one-element local scalar",

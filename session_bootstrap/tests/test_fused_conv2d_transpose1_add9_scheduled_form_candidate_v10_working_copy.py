...
MANIFEST_PATH = HANDWRITTEN_DIR / "scheduled_form_candidate_v10_working_copy_manifest.json"
...
class FusedConv2dTranspose1Add9ScheduledFormCandidateV10WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_the_v10_edit_local_and_isolated(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("Accepted baseline carried forward into this v10 scaffold", text)
        self.assertIn("First real v10 local-proof edit", text)
        self.assertIn("data_pad_h1_seam_carry = T.alloc_buffer", text)
        self.assertIn("data_pad_h1_current = T.alloc_buffer", text)
        self.assertIn("T.int64(32), T.int64(12), T.int64(4), T.int64(2), T.int64(10)", text)
        self.assertIn("T.int64(32), T.int64(4), T.int64(34), T.int64(10)", text)
        self.assertIn('with T.sblock("data_pad_h1_seam_carry_store")', text)
        self.assertIn('with T.sblock("data_pad_h1_current")', text)
        self.assertIn('with T.sblock("compute_update_h1_current")', text)
        self.assertIn("T.where(h_1 == T.int64(0))", text)
        self.assertIn("T.where(h_1 == T.int64(1))", text)
        ...
        self.assertIn("disjoint buffer", text)
        ...
        spec = importlib.util.spec_from_file_location(
            "scheduled_form_candidate_v10_working_copy_tir",
            MODULE_PATH,
        )
    ...
    def test_manifest_records_the_current_checked_in_v10_scaffold_state(self) -> None:
        payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

        self.assertEqual(payload["operator"], "fused_conv2d_transpose1_add9")
        self.assertEqual(
            payload["working_copy_contract"]["path_kind"],
            "diagnostic_scheduled_form_candidate_v10_working_copy",
        )
        self.assertFalse(payload["working_copy_contract"]["performance_claims"])
        self.assertEqual(
            payload["current_edit_state"]["status"],
            "v10_disjoint_second_h1_consumer_buffer_local_proof_failed_against_v7",
        )
        self.assertIn(
            "disjoint current buffer",

...
spec = importlib.util.spec_from_file_location(
    "refresh_fused_conv2d_transpose1_add9_scheduled_form_working_copy",
    SCRIPT_PATH,
)
...
class RefreshFusedConv2dTranspose1Add9ScheduledFormWorkingCopyTest(unittest.TestCase):
    def test_main_writes_distinct_working_copy_and_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_raw:
            ...
            reference_tir = (
                output_dir
                / "fused_conv2d_transpose1_add9_post_db_scheduled_reference_seed_tir.py"
            )
            reference_manifest = (
            ...
            write_text(
                output_dir / "fused_conv2d_transpose1_add9_candidate_v0_tir.py",
                "# candidate v0\n",
            )
            write_text(
                reference_manifest,
                json.dumps(
                    {
                        ...
                        "task_rows": {
                            "task_summary_row": {"rank": 16},
                            "reconstructed_task_row": {"rank": 16},
                        },
                    },
                    indent=2,
                )
            ...
            self.assertIn("older raw pre-compile editable seed", working_copy_tir)
            self.assertIn("checked-in candidate v0", working_copy_tir)
            self.assertIn("from tvm.script import ir as I", working_copy_tir)
            self.assertIn("tir.is_scheduled", working_copy_tir)
    ...
    def test_main_requires_allow_overwrite_for_existing_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_raw:
            ...
            write_text(
                output_dir / "post_db_scheduled_reference_seed_manifest.json",
                json.dumps({"operator": "fused_conv2d_transpose1_add9"}) + "\n",
            )
            write_text(
                output_dir
                / "fused_conv2d_transpose1_add9_scheduled_form_candidate_v1_working_copy_tir.py",

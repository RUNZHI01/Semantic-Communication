...
MODULE_PATH = (
    HANDWRITTEN_DIR
    / "fused_conv2d_transpose1_add9_scheduled_form_candidate_v6_working_copy_tir.py"
)
...
class FusedConv2dTranspose1Add9ScheduledFormCandidateV6WorkingCopyTest(
    unittest.TestCase
):
    def test_working_copy_keeps_the_v6_edit_local_and_isolated(self) -> None:
        text = MODULE_PATH.read_text(encoding="utf-8")

        self.assertIn("Accepted baseline carried forward into this v6 scaffold", text)
        self.assertIn("First real v6 locality/schedule edit", text)
        ...
        self.assertLess(
            text.index("for h_1 in T.serial"),
            text.index("for b_1, c_1 in T.grid"),
        )
        self.assertLess(
            text.index("for b_1, c_1 in T.grid"),
            text.index("for w_1 in T.serial"),
        )
        self.assertIn("T.int64(34)", text)
        self.assertIn(
    ...
    def test_manifest_records_the_current_checked_in_v6_scaffold_state(self) -> None:
        ...
        self.assertEqual(
            ...
            "v6_h1_stripe_staging_on_top_of_v4_applied",
        )
        self.assertIn(
            "only one 34 x 10 stripe is prepared at a time",

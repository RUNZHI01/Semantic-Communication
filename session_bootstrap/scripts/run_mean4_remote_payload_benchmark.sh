...
usage() {
  cat <<'EOF'
Usage:
  run_mean4_remote_payload_benchmark.sh \
    --inference-env <env_file> \
    --local-artifact <path/to/fused_mean4_subtract4_divide4_multiply4_add14_relu3_post_db_swap.so> \
...
- it stages a local mean4 handwritten artifact into a dedicated remote archive
...
LOCAL_ARTIFACT=""
DATABASE_DIR=""
REMOTE_ARCHIVE_DIR="/home/user/Downloads/jscc-test/jscc_staging_handwritten_fused_mean4_subtract4_divide4_multiply4_add14_relu3"
REPORT_ID="mean4_remote_payload_$(date +%Y%m%d_%H%M%S)"
UPLOAD_ONLY=0

#!/usr/bin/env python3
"""Run the transpose2 post-DB schedule-preserving local build path.
...
workflow for `fused_conv2d_transpose2_add12` does not require the user to repeat
...
DEFAULT_OUTPUT_DIR = (
    Path(__file__).resolve().parents[1]
    / "tmp"
    / "transpose2_post_db_swap_local_build"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Operator-specific wrapper for the transpose2 post-db scheduled swap "
            "local build path."
        )
    )
    parser.add_argument(
        ...
        help="Best-staging task_summary.json for transpose2.",
    )
    parser.add_argument(
        "--database-dir",
        type=Path,
        default=DEFAULT_DATABASE_DIR,
        help="Best-staging tuning_logs directory for transpose2.",
    )
    parser.add_argument(
        "--candidate-impl",
        type=Path,

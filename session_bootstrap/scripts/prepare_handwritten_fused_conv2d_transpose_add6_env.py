#!/usr/bin/env python3
"""Prepare staging-safe env files for handwritten fused_conv2d_transpose_add6 work.

This mirrors the narrow handwritten env helper pattern already used for
transpose1:
- start from a checked-in trusted env snapshot template
- redirect current artifact reads to a dedicated handwritten staging archive
- pin the expected SHA for the handwritten candidate
- never mutate trusted current defaults in place
"""
...
DEFAULT_OUTPUT_ENV = (
    ...
    / "handwritten_fused_conv2d_transpose_add6_profile.env"
)
DEFAULT_STAGING_ARCHIVE = "/home/user/Downloads/jscc-test/jscc_staging_handwritten_fused_conv2d_transpose_add6"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "fused_conv2d_transpose_add6 candidate."
        )
    )
    parser.add_argument(
        "--base-env",
    ...
    parser.add_argument(
        "--stage-root",
        default="/home/user/Downloads/jscc-test/jscc_staging_handwritten_fused_conv2d_transpose_add6/infer_outputs",
        help="Optional stage root for output dirs used by remote current reconstruction wrappers.",
    )

#!/usr/bin/env python3
"""Refresh the checked-in scheduled-form v1 working copy for transpose1.
...
RAW_PRECOMPILE_EDITABLE_SEED_FILENAME = f"{OPERATOR_NAME}_editable_seed_tir.py"
CHECKED_IN_CANDIDATE_V0_FILENAME = f"{OPERATOR_NAME}_candidate_v0_tir.py"
...
def render_working_copy_tir(
    *,
    ...
    raw_precompile_seed_path = working_copy_tir_path.parent / RAW_PRECOMPILE_EDITABLE_SEED_FILENAME
    checked_in_candidate_v0_path = working_copy_tir_path.parent / CHECKED_IN_CANDIDATE_V0_FILENAME
    header = [
    ...
    if raw_precompile_seed_path.is_file():
        header.append(
            f"# - distinct from the older raw pre-compile editable seed: {repo_native(raw_precompile_seed_path)}"
        )
    if checked_in_candidate_v0_path.is_file():
        header.append(
            f"# - distinct from the checked-in candidate v0: {repo_native(checked_in_candidate_v0_path)}"
        )
    header.extend(
        [
            "#",
...
def build_working_copy_manifest(
    *,
    ...
    raw_precompile_seed = working_copy_tir_path.parent / RAW_PRECOMPILE_EDITABLE_SEED_FILENAME
    if raw_precompile_seed.is_file():
        related_files["raw_precompile_edit_seed_tir"] = repo_native(raw_precompile_seed)
    checked_in_candidate_v0 = working_copy_tir_path.parent / CHECKED_IN_CANDIDATE_V0_FILENAME
    if checked_in_candidate_v0.is_file():
        related_files["checked_in_candidate_v0_tir"] = repo_native(
            checked_in_candidate_v0
        )
    ...
    return {
        ...
        "notes": [
            "This working copy is derived from the checked-in post-db scheduled reference seed so the first v1 edits start from a scheduled form.",
            "Keep the scheduled reference seed frozen and refresh this working copy when the reference seed changes.",
            "Keep this distinct from the older raw pre-compile editable seed and from the checked-in candidate v0.",

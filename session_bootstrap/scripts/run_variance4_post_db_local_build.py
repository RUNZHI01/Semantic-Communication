#!/usr/bin/env python3
"""Run the variance4 post-DB schedule-preserving local build path.
...
workflow for `fused_variance4_add13_tir_sqrt4` does not require the user to
repeat best-staging task-summary / DB paths by hand.
...
DEFAULT_OUTPUT_DIR = (
    Path(__file__).resolve().parents[1]
    / "tmp"
    / "variance4_post_db_swap_local_build"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Operator-specific wrapper for the variance4 post-db scheduled swap "
            "local build path."
        )
    )
    parser.add_argument(

#!/usr/bin/env python3
"""Run the mean4 post-DB schedule-preserving local build path.
...
workflow for `fused_mean4_subtract4_divide4_multiply4_add14_relu3` does not
require the user to repeat best-staging task-summary / DB paths by hand.
...
DEFAULT_OUTPUT_DIR = (
    Path(__file__).resolve().parents[1]
    / "tmp"
    / "mean4_post_db_swap_local_build"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Operator-specific wrapper for the mean4 post-db scheduled swap "
            "local build path."
        )
    )
    parser.add_argument(
        ...
        help=f"Best-staging task_summary.json for {OPERATOR_NAME}.",
    )
    parser.add_argument(
        "--database-dir",
        type=Path,
        default=DEFAULT_DATABASE_DIR,
        help=f"Best-staging tuning_logs directory for {OPERATOR_NAME}.",
    )
    parser.add_argument(
        "--candidate-impl",
        type=Path,

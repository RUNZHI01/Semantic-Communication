...
- generated_at: 2026-03-01T23:08:52+08:00
...
| 执行策略：远端 TVM prep payload 参数完整 | 不满足 | uses_remote_payload=1; missing_or_placeholder=REMOTE_HOST,REMOTE_USER,REMOTE_PASS,REMOTE_TVM_PYTHON,REMOTE_TVM_PRIMARY_DIR,REMOTE_TVM_JSCC_BASE_DIR,REMOTE_FULL_BASELINE_ARCHIVES,REMOTE_FULL_CURRENT_ARCHIVES; REMOTE_HOST=replace_with_snapdragon_ip; REMOTE_TVM_PRIMARY_DIR=replace_with_snapdragon_primary_archive_dir |
...
| 执行策略：单变量实验字段已定义 | 满足 | DAILY_SINGLE_CHANGE=switch snapdragon prep payload from mock commands to real TVM runtime/db probe while keeping phytium target fixed |
...
- 补齐 REMOTE_* 的骁龙连接与归档路径，避免跑到 mock 或空配置。

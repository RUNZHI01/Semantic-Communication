...
灾后应急、无人机巡检与现场机器人作业等极端弱网场景下，原始图像回传面临带宽不足、链路不稳定和现场无人值守等多重挑战。本文构建了一套部署在飞腾多核异构平台上的安全语义图像回传系统：上位机完成图像语义编码与信道扰动模拟，将低负载语义特征发送至飞腾端；Linux 主核利用 TVM/MNN 双引擎完成高性能图像重建、显示与存储；RTOS 从核基于 OpenAMP 与 Linux 主核通信，提供独立的作业准入、心跳监护与安全停机能力，避免高负载推理挤占安全判定路径。

系统在飞腾真机上已形成完整闭环。4 核 Linux 性能模式下，TVM 主线通过编译器级优化将单张图像端到端重建时间从 1850 ms 降至 230 ms；结合 big.LITTLE 异构流水线后，单张中位时间进一步压到 134.6 ms。3 核 Linux + RTOS 演示模式下，系统保留 1 核运行 RTOS 控制面，在 OpenAMP 五状态状态机下完成作业准入、心跳监护与安全停机闭环（串行重建中位时间 345.6 ms，流水线 249.4 ms），并通过三项故障注入验证。本文在 MetaSchedule 自动调优基础上，通过 Profiling 定位关键热点算子，并依次尝试 ARM 官方 ACL 算子库替换与手写算子改写；结果表明手写算子在端到端性能上明显优于 ACL 算子库与 MetaSchedule 优化效果。其中转置卷积算子通过 bias 融合消除中间缓冲，方差计算算子通过逐通道本地缓冲复用减少全帧写回，归一化算子依次完成参数提取、仿射预计算和 NEON 向量归约。针对动态尺寸灵活部署需求，MNN 路线支持 300 张不同分辨率图像的直接处理（端到端总耗时 98.2 秒，平均 327.3 ms/张），避免了 TVM 静态编译对固定输入形状的约束。为满足极端场景下的通信安全与可靠性需求，系统在 OpenAMP 安全控制协议之上进一步引入了后量子加密技术，确保语义特征传输的前向安全性。

**主要创新与贡献：**

1. **数据面/控制面分离的多核异构架构。** Linux 主核承担 TVM/MNN 推理与重建，RTOS 从核通过 OpenAMP 提供独立的准入、监护与安全停机能力，避免高负载推理挤占安全判定路径。
2. **基于 TVM 增量调优、Profiling 与算子级手写迭代的深度优化链路。** 构建了“编译目标收敛 → 增量调优 → 热点定位 → ACL 算子库替换对比 → 手写改写 → 整模型验证”的完整闭环；其中转置卷积算子采用中间缓冲消除与 bias 融合，方差计算算子采用逐通道本地缓冲复用，归一化算子采用参数提取、仿射预计算与 NEON 向量归约，使端到端重建性能提升 87.6%。手写优化版本在端到端性能上明显优于 ARM 官方 ACL 算子库与 MetaSchedule 自动调优效果。
...
5. **后量子加密与 USRP 安全传输。** 系统采用符合国家标准的后量子密码算法对语义特征传输进行加密保护，结合 USRP 硬件实现真实信道下的安全回传，满足极端弱网环境下的前向安全性需求。
6. **北斗高精度定位融合。** 引入百度提供的北斗高精度定位服务，将无人机/机器人巡检的地理坐标信息与语义回传数据同步关联，实现“看得清 + 定得准”的应急巡检能力。
7. **基于 Electron 的可视化演示系统。** 开发 Electron 桌面端演示界面，将语义回传、飞腾重建、OpenAMP 控制与故障收敛的完整闭环实时可视化，使评审可直接观察作业准入、心跳监护、异常触发与安全停机的真实执行过程。

## 目录

- [第十届全国大学生集成电路创新创业大赛](#第十届全国大学生集成电路创新创业大赛)
  - [摘要](#摘要)
  - [目录](#目录)
  - [1. 系统定位与总体方案](#1-系统定位与总体方案)
    - [1.1 应用场景与需求分析](#11-应用场景与需求分析)
    - [1.2 系统总体架构](#12-系统总体架构)
    - [1.3 赛题任务映射](#13-赛题任务映射)
  - [2. 技术背景](#2-技术背景)
  ...
  - [3. 系统设计与实现](#3-系统设计与实现)
    ...
    - [3.3 安全可靠性设计——OpenAMP 控制面](#33-安全可靠性设计openamp-控制面)
    ...
    - [3.4 系统演示界面与演示流程](#34-系统演示界面与演示流程)
      - [3.4.1 演示流程](#341-演示流程)
      - [3.4.2 界面元素与可验证证据](#342-界面元素与可验证证据)
...
原始图像回传在弱网条件下极易因带宽不足而失效；单核推理演示也无法体现多核协同与安全控制。语义通信的价值在于：不直接传输原始像素，而是发送端提取语义特征、接收端完成图像重建，从而把通信压力从像素级降到语义级[4][5]。

本系统将落点设定为“为无人机/机器人巡检任务提供弱网下可运行、可管控、可安全停机的语义图像回传底座”。此外，实际巡检场景中不仅有标准分辨率的图像，还经常面临多源异构传感器传来的动态分辨率图像，这就要求系统在追求极致性能的同时兼顾动态尺寸的灵活部署。

在上述场景下，通信安全同样不可忽视：前端节点通过公共信道或无线链路回传数据时，面临信道窃听、中间人攻击与数据篡改等威胁；传统加密算法在量子计算发展背景下存在长期安全隐患。因此，系统除满足实时性能与灵活部署外，还必须具备**通信安全**能力。本文进一步引入后量子密码技术，在语义特征传输层叠加符合国家标准的抗量子加密保护，确保极端弱网环境下的前向安全性。
...
**控制面**负责安全管控：RTOS 从核基于 OpenAMP 与主核通信，承担作业准入判定、心跳监护、工件验签和安全停机等职责。OpenAMP（Open Asymmetric Multi-Processing）是开源非对称多处理框架，通过 RPMsg 实现 Linux 主核与 RTOS 从核间的轻量级核间通信。控制面只传输作业编号、校验信息、心跳等小消息，不参与大数据流传输。

![图 1.1 系统总体架构图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_system_arch_cn_20260405.png)
...
图 1.3 给出系统状态的横向摘要：从弱网图像输入、上位机语义编码与后量子加密，到飞腾端数据面/控制面协同，再到最终的演示界面与结果输出，整条链路和关键结果汇总在同一页中。

![图 1.3 飞腾多核异构安全语义回传系统摘要图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_cover_summary_cn_20260405.png)

> **图 1.3** 飞腾多核异构安全语义回传系统摘要图（一张图同时概括链路、角色分工与关键结果，适合作为论文首页或答辩首页的摘要页）

实验上位机使用 2022 款联想拯救者 Y9000P，配置 NVIDIA GeForce RTX 4060 Laptop GPU（8GB）和 13th Gen Intel Core i7-13700H 处理器。上位机通过安全连接与飞腾派建立通信，将语义特征发送至板端触发解码执行。

### 1.3 赛题任务映射

**具体完成方式：**

- **基础任务（上位机与飞腾端协同应用）：** 上位机部署语义编码器完成图像特征提取，通过安全链路将语义特征发送至飞腾端；飞腾端 TVM/MNN 引擎完成图像重建、实时显示与历史回放。系统已完成 300 张图像端到端真机重建验证（见 4.1.3 节）。
...
- **高阶任务（Linux 主核 + RTOS 从核协同，实时响应与安全控制）：** RTOS 从核承担工件验签、作业准入、心跳监护与安全停机职责，与 Linux 主核的 TVM 推理任务解耦，避免高负载推理挤占安全判定路径。此外，系统引入后量子加密技术对语义特征传输进行加密保护，并结合 USRP 硬件实现真实信道下的安全回传。三项故障注入测试（FIT-01/FIT-02/FIT-03）全部真机通过（见 3.3.3–3.3.4 节）。
...
![图 2.1 GAN-based JSCC 系统结构图](制图/图2.1/成品/fig2_1_gan_jscc_ieee_v5.png)
...
3. **安全传输：** 通过安全通道将语义特征发送至飞腾端。256×256 RGB 原始图像约 196 KB，经语义编码后的 32×32×32 latent 张量（FP32）约 128 KB，压缩比约 1.5 倍。
4. **作业准入（演示模式）：** 飞腾主核通过 OpenAMP 向从核发送作业请求，从核完成工件验签与准入判定。
5. **语义重建：** 飞腾主核调用 TVM 或 MNN 执行 Generator 推理，完成图像重建。
6. **输出与存储：** 重建结果送显、PNG 输出与结果归档。
...
**3 核 Linux + RTOS 演示模式**下，通过 `remoteproc` 拉起 RTOS 控制面，Linux 可用核心数减为 3 核，但可完整展示 OpenAMP 安全控制能力。该模式已跑通全部 300 张图像推理与 OpenAMP 完整闭环（五类消息 + 三项 FIT 测试），主要用于安全控制演示而非追求最优吞吐。由于 `remoteproc` 占用一个 CPU 核心，该模式下的推理时延略高于 4 核性能模式，详细数据见 4.1.3 节。该模式的核心价值在于通过 RTOS 从核提供独立于 Linux 数据面的安全控制通路，其准入、监护与停机机制详见 3.3 节。

### 3.3 安全可靠性设计——OpenAMP 控制面

在弱网巡检与应急回传场景中，系统面临多重安全可靠性威胁：高负载推理任务可能挤占安全判定路径，导致心跳监护与准入机制失效；信道不稳定可能引发心跳丢失，造成从核误判主核失联；非法模型或越界参数若未经校验直接执行，将产生不可预期的推理结果；Linux 主核在复杂软件栈下运行，实时性与确定性不足，难以承担毫秒级安全响应。上述漏洞若不加以隔离，安全控制逻辑将与数据面推理竞争 CPU 时间，最终丧失对异常工况的及时收敛能力。

OpenAMP（Open Asymmetric Multi-Processing）为此提供了独立的控制面通路：RTOS 从核通过 RPMsg 与 Linux 主核通信，专责作业准入、运行监护与安全停机，不介入数据面的 TVM/MNN 推理。将安全判定从高负载 Linux 数据面中剥离，形成独立、可预期的控制通路。
...
- **SAFE_STOP → READY：** 故障记录输出后，系统自动回到可观测的安全态，等待下一轮作业。
...
![图 3.1 OpenAMP 控制状态机与消息转移图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_openamp_state_machine_cn_20260405.png)
...
消息在 Linux 主核与 RTOS 从核之间按固定时序流转：先查询状态，再执行准入，允许后进入心跳监护与任务完成上报；心跳丢失或人工停机时，控制面转入 SAFE_STOP/FAULT。
...
消息大小统计：JOB_REQ 为 48 字节，HEARTBEAT 为 14 字节，STATUS_RESP 为 18 字节。相比图像数据（256×256 RGB 约 196 KB），控制消息带宽占比低于 0.02%，满足“控制面轻载”的设计目标。

![图 3.2 控制协议与消息时序](制图/图3.2/成品/paper_fig3_2_sequence_final_v2.png)
...
**停机收敛流程：** Linux 收到安全停机判定后，中止当前重建、丢弃未完成输出并回收任务上下文。系统进入可观测的安全态（SAFE_STOP），`last_fault_code` 记录停机原因，`total_fault_count` 累加，可通过 STATUS_REQ 查询。故障记录输出后系统回到 READY，支持下一轮作业调度。
...
FIT-03 的验证过程特别值得说明，其证据链覆盖了“发现缺口 → 定位原因 → 修复 → 复验通过”的完整闭环，而非一次通过的简单测试：

**发现缺口。** 首轮故障注入时，探针故意在 RUNNING 状态下停发心跳，同时通过串口持续读取从核状态。观察发现：停发 10 秒后，从核状态仍为 RUNNING，`last_fault_code` 为 0x0000，系统未触发任何收敛动作。这意味着从核并未实现心跳超时检测机制——一旦 Linux 主核因崩溃或调度异常停止上报心跳，从核将永远等待，无法进入 SAFE_STOP，构成真实的安全缺口。

**定位原因。** 查阅 RTOS 固件代码后发现，早期版本仅在消息处理函数中被动响应 HEARTBEAT 消息，未维护独立的时间窗或超时计数器。因此心跳丢失不会被检测，状态机缺少 RUNNING → SAFE_STOP 的超时转移路径。

**修复。** 在固件中补充惰性看门狗机制：定义预期心跳周期 500 ms、超时阈值 2000 ms、连续丢失阈值 4 次。参数选取依据为：500 ms 周期在实时监护与系统负载之间取得平衡；2 s 超时（4 个周期）给 Linux 调度抖动和网络延迟留出余量，避免偶发丢包导致误停机；4 次连续丢失则确保只有在主核确实失联而非短暂阻塞时才触发收敛。看门狗采用惰性设计——不在后台持续计时，而是在每次消息处理循环中检查当前时间与上次心跳时间之差，仅在 RUNNING 状态下生效，避免空闲态 overhead。

**复验。** 修复后使用同一探针重新执行 FIT-03：连续停发心跳，记录从核状态与故障码变化时序。复验结果显示：停发第 4 个心跳周期后（约 2 s），从核状态由 RUNNING 转移至 SAFE_STOP，`last_fault_code` 正确记录为 0xF003（HEARTBEAT_TIMEOUT），`total_fault_count` 累加 1。随后通过 STATUS_REQ 查询，确认系统处于可观测安全态；再次发送 JOB_REQ，系统从 SAFE_STOP 回到 READY，支持下一轮作业。该复验重复执行 10 轮，全部通过。

### 3.4 系统演示界面与演示流程

演示界面的核心目标不是展示 UI 设计，而是将“语义回传—飞腾重建—OpenAMP 控制—故障收敛”的完整闭环实时可视化，使评审能够直接观察到作业准入、心跳监护、异常触发与安全停机的真实执行过程。

系统提供 Electron 桌面端演示界面，共享后端 API。
...
7. **结果确认：** 查看重建结果与控制面消息日志，确认端到端闭环完成。

两种模式在 CPU 分配、功能侧重和性能基准上存在明确区分：4 核性能模式用于展示最大吞吐能力，3+1 演示模式用于展示安全控制能力。由于 `remoteproc` 占用一个 CPU 核心，两种模式的性能数字不可直接对比。

![图 3.3 双模式边界图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_mode_boundary_cn_20260405.png)
...
![图 3.4 Electron 演示界面证据映射图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_demo_workflow_cn_20260405.png)
...
![图 3.5 Electron 演示系统主界面截图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_demo_dashboard_ui_cn_20260405.png)
...
![图 3.6 Electron 演示系统关键细节界面](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_demo_detail_panels_cn_20260405.png)
...
TVM（Tensor Virtual Machine）是开源深度学习编译器[10]，通过多层次中间表示（Relax → TE → TIR）和自动调优技术（MetaSchedule）[11]实现模型的高效跨硬件部署。本节围绕编译框架优化、算子级手写改写和整模型性能评估展开。

![图 4.1 TVM 编译优化流程图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_tvm_opt_flow_cn_20260405.png)
...
**（2）编译目标收敛。** 在相同数据库与相同推理路径下，对四种目标配置开展真机比较：
...
**（3）基于历史数据的增量调优。** 导入 ONNX 至 Relax IR 后，使用 `relax.get_pipeline` 与 MetaSchedule 组织编译优化。与初始版本的零预算重编译路径不同，优化后版本引入增量调优策略：以历史调优数据库中的有效记录作为种子，在新目标与新的运行时条件下继续执行 1500 次非零预算搜索，搜索空间的调度策略采用贝叶斯优化。
...
# 增量搜索配置：1500 次 trials，采用贝叶斯优化调度策略
config = ms.TuneConfig(
    strategy="evolutionary",
    num_trials=1500,             # 非零预算搜索
...
|-----|------------------:|------------------:|---------:|
| transpose1 | 28500 | 24275 | 提升 14.8% |
| transpose2 | 23800 | 20235 | 提升 15.0% |
| conv2d3_add15 | 14200 | 11801 | 提升 16.9% |

该策略继承了历史对高价值调度区域的先验知识，同时允许在新环境下完成局部再学习。搜索完成后，编译并导出最终优化产物，同时对产物进行 SHA-256 校验以确保身份一致性。

**（4）真实端到端重建验证。** 补齐了完整的重建执行器，使评测不再停留在纯推理负载层面，而是完整覆盖 latent 读取、AWGN 扰动注入、VM 执行、图像解码与结果输出的真实链路。

#### 4.1.2 基于 Profiling 的算子级手写优化

编译框架级优化完成后，继续对热点算子做 TIR 手写改写。流程为：Profiling 定位热点 → 基于 MetaSchedule 基线做 TIR 层改写 → 回接整模型 → 板端 A/B 验证。每个改写候选须通过局部正确性校验（`allclose(1e-6)`）、整模型产物导出、SHA-256 校验和板端真机 A/B 四道关卡。
...
|------------|---------|------------------:|----------:|-----------:|-----:|
| transpose（转置卷积） | `transpose1` / `transpose2` / `transpose_add6` | 55.0 / 43.9 / 40.9 | 15.3% / 12.3% / 11.4% | 48.0 / 38.7 / 35.0 | 提升 12.7% ~ 14.5% |
| variance（方差计算） | `variance3` | 3.58 | 1.0% | 2.74 | 提升 23.4% |
| mean4（归一化 + affine + ReLU） | `mean4` | 3.10 | 0.9% | — | 见下文迭代 |

> **表 4.1** 关键热点的 family 分类与手写优化效果

![图 4.2 关键算子图内性能差异与热点占比](制图/图4.2/成品/paper_fig4_2_operator_hotspot_strict_ieee.png)
...
该改写保持了 MetaSchedule 已收敛的 tiling 结构不变，仅修改了初始化值和输出目标。三段 transpose 经此改写后，图内时间相对基线分别下降 12.7%、11.9% 和 14.5%。
...
该改写消除了全部 4 个全帧中间缓冲，流量从约 31 MB 降至约 6 MB。
...
**中间产物的 codegen 检查与方向校正。** 为了确定下一步优化方向，对 v5 产物的 AArch64 反汇编进行了检查。结果表明，epilogue（affine + ReLU）已经被 LLVM 成功优化为 NEON 向量指令：
...
mean4 的热路径从 baseline 的"7 个独立 pass + 标量全路径"，经参数提取、仿射预计算、向量归约三步迭代，达到"reduction 侧和 epilogue 侧均为 NEON 向量化"。表 4.2 给出各阶段的 codegen 演进：

| 优化阶段 | reduction 侧 codegen | epilogue 侧 codegen | 关键改写 |
|-----|----------------------|---------------------|---------|
| 基线实现 | 标量 | 标量（7 个独立 pass） | — |
| 策略一 | 标量 | 标量（单次遍历融合） | 参数提取 + pass 融合 |
...
**（5）验证闭环。** 每个手写候选在板端验证前均须通过：局部输出与 PyTorch 参考值的 `allclose(1e-6)` 容差校验、整模型产物导出与 SHA-256 一致性校验、以及同板态下的纯推理与重建时间 A/B 对比。手写多算子整合路线的板端验证结果见 4.1.3 节表 4.3。
#### 4.1.3 整模型性能结果
...
端到端重建测试中，初始版本与优化版本各生成 300 张 PNG 重建图像，确保对比口径一致。
...
![图 4.3 纯推理与串行重建性能对比](制图/图4.3/成品/paper_fig4_3_big_little_pipeline_strict_ieee_v2.png)

> **图 4.3** 纯推理与串行重建性能对比（OpenAMP 三核统一板态）

手写优化版本在纯推理时间、串行重建时间、流水线重建时间和吞吐提升四项指标上均优于 MetaSchedule 优化版本。
...
![图 4.4 TVM trusted current 的多 SNR 鲁棒性](制图/图4.4/成品1/paper_fig4_4_snr_robustness_strict_ieee_v2.png)

> **图 4.4** TVM trusted current 的多 SNR 鲁棒性（真机 300 张图像复测）

重建时间在 228~234 ms/image 范围内保持稳定，对 SNR 变化不敏感；图像质量随信道条件改善而稳步提升。

![图 4.5 重建效果对比](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_reconstruction_gallery_cn_20260405.png)
...
![图 4.6 TVM 主线结果总览](制图/图4.6/成品/paper_fig4_6_tvm_overview_ieee_v3.png)
...
飞腾派 CPU 采用 big.LITTLE 异构架构（2 个 1.8 GHz Cortex-A72 大核 + 2 个 1.5 GHz Cortex-A55 小核）。串行模式下，推理、数据加载与结果输出依次执行，大核在等待 I/O 时处于空闲状态。为实现吞吐最大化，系统设计了计算与 I/O 分离的异构流水线。

**流水线调度策略。** 通过 Linux `sched_setaffinity` 系统调用将不同角色的进程绑定到对应核心组：大核（Cortex-A72）专注 TVM 推理计算，小核（Cortex-A55）负责 latent 文件预加载和 PNG 后处理。具体而言，系统启动三个协同进程：预加载进程（小核）从磁盘读取下一批 latent 张量并写入共享队列；推理进程（大核）从队列取数据执行 TVM VM 推理；后处理进程（小核）将推理结果转换为 PNG 格式并输出。三个阶段形成时间重叠的流水线——当大核推理第 $n$ 张图时，小核同时在预加载第 $n+1$ 张并输出第 $n-1$ 张的结果。
...
![图 4.9 异构大小核流水线性能对比](制图/图4.9/成品/fig_big_little_pipeline.png)

> **图 4.9** 异构大小核流水线性能对比（OpenAMP 三核统一板态）

手写优化版本在串行重建时间、流水线重建时间和吞吐提升三项指标上均优于 MetaSchedule 优化版本。

### 4.2 MNN 动态尺寸路线
TVM 经底层调优后吞吐最高，但静态图编译限制了输入分辨率必须为 256×256。为处理多源传感器动态分辨率输入，引入 MNN 作为动态尺寸旁路。
MNN（Mobile Neural Network）是阿里巴巴开发的轻量级推理引擎，其核心创新包括几何计算机制（将转换和复合算子分解为原子算子，减少 46% 的优化工作量）和半自动搜索机制（运行时快速识别最优算子实现）[7][8]。

![图 4.7 MNN 架构示意图](制图/图4.7/成品/fig_mnn_arch.png)
...
![图 4.8 MNN 动态尺寸路线关键配置对比](制图/图4.8/成品/fig_mnn_benchmark_optimized.png)
...
**双引擎协同定位：** TVM 与 MNN 服务于不同部署需求。TVM 在固定分辨率批量处理下提供极致性能，MNN 在动态分辨率场景下提供即时适配能力。两者共享上位机编码流程和飞腾端输出管线，通过运行时配置切换工作模式。



**推理框架选型依据。** 飞腾派端侧推理的主流选择包括 TVM、MNN、NCNN 和 ONNX Runtime 等。TVM 通过 Relax IR 和 MetaSchedule[11]提供从计算图到目标代码的完整优化链路，适合围绕 Cortex-A72 做深度定制；MNN 对动态输入形状支持成熟，模型转换链稳定，适合补充 TVM 的静态图限制。NCNN 缺少编译器级自动调优，ONNX Runtime 缺少面向特定微架构的优化空间。

实际部署中，双引擎的选择不只看吞吐。TVM 在固定形状、固定产物和多核异构流水线条件下给出当前最强性能主口径；MNN 则在混合尺寸输入、无需额外缩放的部署场景中更具工程灵活性。最终采用 TVM 负责固定形状性能、MNN 负责动态尺寸灵活部署的双轨方案。

![图 4.10 双引擎部署定位与端侧性能关系](制图/图4.10-enhanced/成品/fig_framework_positioning.png)

> **图 4.10** 双引擎部署定位与端侧性能关系（TVM 负责固定形状性能主口径，MNN 负责动态尺寸灵活部署；OpenAMP 演示模式的控制/安全结论不与本图 latency 混写）

**与传统图像压缩的对比。** 语义通信与传统图像压缩（如 JPEG、H.265）的根本区别在于传输内容：传统方案压缩并传输像素级数据，语义通信则在发送端提取语义特征、在接收端重建图像。在本系统中，Encoder 将 256×256 图像编码为 32×32×32 的 latent 张量（约 128 KB FP32），相比原始 RGB 图像（约 196 KB）压缩比约 1.5 倍；而在更低 SNR 条件下，语义通信的重建质量衰减远缓于传统压缩方案——文献[6]的实验表明，在 SNR=1 dB 的极端弱网条件下，GAN-based JSCC 的 PSNR 仍可达 29 dB 以上，而传统 JPEG + 信道编码方案在同等条件下已基本不可用。这一特性使语义通信在弱网巡检场景中的优势得以凸显。

![图 4.11 传统压缩与语义通信的弱网对比](制图/图4.11/成品/fig_semantic_vs_traditional.png)

> **图 4.11** 传统压缩与语义通信的弱网对比（同时对比“传什么”和“弱网下会怎样”；大小对照来自本系统 `256×256 RGB` 输入与 `32×32×32 latent`，弱网鲁棒性结论引自文献[6]）

第 4 章从 TVM 固定形状性能主路线、big.LITTLE 异构流水线加速、MNN 动态尺寸旁路到双引擎选型依据，完成了推理框架层面的完整论证。
...
![图 5.1 系统关键性能跃迁与最新 OpenAMP 三核 handwritten follow-up 定位](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_performance_ladder_cn_20260406.png)
...
![图 5.2 答辩证据包结构图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_evidence_bundle_cn_20260405.png)

> **图 5.2** 答辩证据包结构图（将面向评审的材料按“总判定、live 状态、安全验证、性能质量、演示话术”五类组织，避免现场翻阅时临时拼凑证据）

图 5.2 给出评审追问时的证据入口；系统层面的概括则回到数据面、控制面与展示面的协同关系。TVM 与 MNN 分别承担固定形状极致性能与动态尺寸灵活部署的角色，共同服务于飞腾端数据面；OpenAMP 控制面则提供准入、监护和安全停机能力。数据面与控制面分离，保证了高性能结果，也使多核异构协同的角色更加清晰。

![图 5.3 弱网安全语义回传系统闭环总览图](Semantic-Communication/session_bootstrap/reports/figures/paper_fig_system_closure_overview_cn_20260405.png)

> **图 5.3** 弱网安全语义回传系统闭环总览图（把数据面、控制面、性能主口径与答辩证据入口压到同一页，直接对应“传得回、跑得快、用得稳”的系统级结论）

**系统的扩展性与未来演进：**

1. **后量子加密算法的完整端侧部署与性能评估**：当前后量子加密已在上位机与传输层完成验证，后续需在飞腾派端侧完成轻量级后量子密码模块的集成，并评估其对推理时延与内存占用的实际影响。
2. **北斗高精度定位数据与语义回传的融合验证**：百度北斗定位服务已完成接口对接，下一步需在真实巡检场景中采集带有地理坐标的语义回传数据，验证位置-图像关联的完整性与精度。
3. **USRP 真实信道下的端到端联调**：当前系统已在 AWGN 模拟信道下验证鲁棒性，后续计划接入 USRP 硬件平台，在真实无线衰落环境中完成后量子加密语义回传的全链路联调。
